...
def import_rs_binary_file(full_file_path: 'Full folder path and file name.bin'):
    ...
    rs_data.pll1_frequencies_kHz = np.frombuffer(binary_string[start_index:stop_index], '<i')

    # Pll2 (LO) frequencies [kHz]
    start_index = stop_index
    stop_index = start_index + rs_data.num_steps*pll_freq_point_bytes
    rs_data.pll2_frequencies_kHz = np.frombuffer(binary_string[start_index:stop_index], '<i')

    # Pll1 (TX) Power Level
    start_index = stop_index
    stop_index = start_index + rs_data.num_steps*pll_power_level_point_bytes
    rs_data.pll1_power_levels = np.frombuffer(binary_string[start_index:stop_index], '<i')

    # Pll2 (LO) Power Level
    start_index = stop_index
    stop_index = start_index + rs_data.num_steps*pll_power_level_point_bytes
    rs_data.pll2_power_levels = np.frombuffer(binary_string[start_index:stop_index], '<i')

    # Timestamps.
    start_index = stop_index
    stop_index = start_index + rs_data.num_total_frames*time_stamp_size_bytes
    rs_data.time_stamps = np.frombuffer(binary_string[start_index:stop_index], '<H') # H = unsigned short
    ...
    start_index = stop_index
    radargram_re_im_parts = np.frombuffer(binary_string[start_index:], dtype='<f')
