...
class RsLstmModel(torch.nn.Module):
    ...
    def forward(self, X_batch, x_lengths):
        '''
            ...
            @return y_pred: discrete probability distribution of classes for the last output
                            of each batch item [batch_size, num_classes]
        '''
        ...
        if(self.is_bidirectional):
            h_T = torch.cat((h_T[-1,:,:], h_T[-2,:,:]), dim=-1) # Note: see bilstm_output_shapes.pdf for an explanation.
        else:
            h_T = h_T[-1,:,:]
    ...
    def infer(self, sequence):
            '''
                Inference alternative, where a single 2-d sequence is the input without batching.
                @param X_batch: [2-d tensor] containing the input sequence with dim. [seq_length, input_size]
                @return y_pred: [1-d vector] discrete probability distribution of classes for the last output
                                of each batch item [num_classes]
            '''
            
            # Batch size is fixed to 1.
            h_0 = torch.zeros(self.num_layers*self.num_directions, 1, self.hidden_size, requires_grad=True).to(self.device)
            c_0 = torch.zeros(self.num_layers*self.num_directions, 1, self.hidden_size, requires_grad=True).to(self.device)
                   
            sequence = sequence.unsqueeze(dim=0).to(self.device) # Add the batch dimension as required for the lstm input layer.

            output, (h_T, c_T) = self.lstm(sequence, (h_0.detach(), c_0.detach())) 
            ...
            # Apply the softmax layer.
            h_T = torch.nn.functional.log_softmax(h_T, dim=1).squeeze(dim=0)
