...
if __name__ == "__main__":
    ...
    num_model_evaluations = 1 # Number of hyperparameter search runs.
    ...
    # Select the variable hyperparameters subject to optimization. 
        ...
        learning_rate = c[np.random.randint(low=0, high=num_learning_rates-1)].item() # NOTE: FIXED FOR FINAL MODEL TRAINING
        ...
        '''
            # TEST INFERENCE
            test_input = torch.randn((100,201),dtype=torch.float32)
            output = rs_lstm_model.infer(test_input)
            # END TEST INFERENCE
        '''

        # Save the model.
        model_file_name = "lstm_optim_model_" + timestamp + "_" + str(run_index) + ".pt"
        torch.save(trainer.training_results.best_model, model_file_name)

        # Also save the trace for C++ import.
        trace_file_name = "lstm_optim_trace_" + timestamp + "_" + str(run_index) + ".pt"
        example_length = torch.tensor([100],dtype=torch.int64)
        example_input = torch.rand((1,100,num_features))
        output = trainer.training_results.best_model(example_input, example_length)
        traced_script_module = torch.jit.trace(trainer.training_results.best_model, (example_input, example_length))
