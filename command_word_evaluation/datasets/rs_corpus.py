...
def get_corpus_full_file_paths():
    ...
    local_pc_path = "C:\\Users\\chris\\Documents\\Institut\\corpora\\command_word_recognition_monopole_2\\binCorpus\\"
    ...
    def load_files(self, path_to_corpus, is_verbose=False):
        ...
        for subject_index in range(self.num_subjects):
            ...
            for session_index in range(len(session_ids)):
                path_to_files = os.path.join(path_to_sessions, session_ids[session_index], 'radar_files')
        ...
        for subject_id in self.subject_ids:
            ...
            for session_id in self.session_ids[subject_id]:
                ...
                rd_full_file_path = os.path.join(path_to_corpus, subject_id, session_id, 'radar_files')
                ...
                try:
                    ...
                    print("Could not load the label files %s, does the folder/files exist?." % (labels_full_file_Path))
                num_label_files = len(labels_directory)
                if(num_label_files <= 0):
                    raise ValueError("No label files found under %s." % (labels_full_file_Path))
        
                if(num_rs_data_files != num_label_files):
                    raise ValueError("Error: {} rs data files, but {} label files.".format(num_rs_data_files, num_label_files))
                else:
                    if(is_verbose):
