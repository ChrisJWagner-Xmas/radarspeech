...
class ParameterizedTransformFunction():
    ...
    def __init__(self, spectra, transforms, freq_start_index, freq_stop_index, norm_interval=[[-1,1]], ltas_mag=[None,None]):
        ...
        self.sequence_indices = [] # Maps: S12 -> 0, S32 -> 1
        self.allowed_spectrum_keywords = ['S12', 'S32']
        self.allowed_transform_options = ['mag','mag_db','phase','mag_delta','mag_db_delta','phase_delta','impulse_response']
        # Convert LTAS to tensors.
        self.ltas_mag = ltas_mag
        for ltas_index, _ in enumerate(ltas_mag):
            if ltas_mag[ltas_index] is not None:
                self.ltas_mag[ltas_index] = torch.from_numpy(ltas_mag[ltas_index])

        self.num_pairs = 0 # Number of [spectra,transform]-pairs passed to the transform function.
        self._mag_min_value_db = -45.0 # approximately the noise floor of the current radar hardware. 
        ...
        for keyword in self.spectra:
            ...
            if not(any(is_key_valid_list)):
                raise ValueError("Error: allowed keys in spectra_and_transforms are: %s. Passed: %s" % (self.allowed_spectrum_keywords, keyword))
        ...
        for keyword in self.transforms:
            ...
            if not(any(is_transform_valid_list)):
                raise ValueError("Error: allowed keys in spectra_and_transforms are: %s. Passed: %s" % (self.allowed_transform_options, keyword))
        
        # The impulse response must not be combined with frequency features atm. Check this:
        for keyword in self.transforms:
            if(keyword == 'impulse_response' and len(self.transforms) > 1):
                raise ValueError("Error: impulse response must not be used with other frequency features atm (transform list has length %d)" % (len(self.transforms)))

        self.num_pairs = len(self.spectra)

        # Check the number of normalization intervals.
        if(len(self.norm_interval) is not self.num_pairs):
            raise ValueError("Error: %d normalization intervals provided for %d transform pairs." % (len(self.norm_interval), self.num_pairs))
    ...
    def transform_sequences(self, sequence_list):
            ...
            for pair_index in range(self.num_pairs):
                ...
                sequence_index = self.sequence_indices[pair_index];

                # Crop the sequence.
                sequence = sequence_list[sequence_index][:, self.freq_start_index: self.freq_stop_index]

                # Apply basic transform(s).
                if(self.transforms[pair_index] == 'mag_delta'):
                    sequence = calculate_delta_sequence(sequence)
                    sequence = torch.abs(sequence)

                if(self.transforms[pair_index] == 'phase_delta'):
                    sequence = calculate_delta_sequence(sequence)
                    sequence = torch.angle(sequence)                     

                if(self.transforms[pair_index] == 'mag_db_delta'):
                    sequence = calculate_delta_sequence(sequence)
                    sequence = 20*torch.log10(torch.abs(sequence))
                    sequence[sequence < self._mag_min_value_db] = self._mag_min_value_db 

                if(self.transforms[pair_index] == 'mag'):
                    sequence = torch.abs(sequence)
                    # Subtract the LTAS from the magnitude spectra if provided.
                    if(self.ltas_mag[sequence_index] is not None):
                        sequence = sequence - self.ltas_mag[sequence_index][self.freq_start_index: self.freq_stop_index]    

                if(self.transforms[pair_index] == 'mag_db'):
                    sequence = 20*torch.log10(torch.abs(sequence))
                    sequence[sequence < self._mag_min_value_db] = self._mag_min_value_db 

                if(self.transforms[pair_index] == 'phase'):  
                    sequence = torch.angle(sequence)

                if(self.transforms[pair_index] == 'impulse_response'):
                    sequence = calc_impulse_response(sequence)

                '''
                # Always normalize *each frame* to [a,b]
                    max_values = torch.max(sequence,1).values # normalize across the feature dim.
                    ...
                    norm_factor = (self.norm_interval[pair_index][1] - self.norm_interval[pair_index][0])
                    sequence = norm_factor*sequence + self.norm_interval[pair_index][0]
                '''
                
                # Always normalize *each sequence* to [a,b]
                max_value = torch.max(sequence)
                min_value = torch.min(sequence)
                sequence = (self.norm_interval[pair_index][1] - self.norm_interval[pair_index][0]) \
                                    *(sequence - min_value)/(max_value - min_value) + self.norm_interval[pair_index][0]
                
                # if several spectra are used as input features, concatenate them along the feature axis.
                feature_vector[:, pair_index*self.freq_stop_index:(pair_index+1)*self.freq_stop_index] = sequence
...
class Trainer():
    ...
    def fit_model(self, model, optimizer, train_data_loader, validation_data_loader, patience=20):
        ...
        for epoch in range(self.num_epochs):
            ...
            if(self.is_verbose):
                print("Epoch %d, Validation accuracy %1.3f, Train accuracy: %1.3f" % (epoch, validation_accuracy, train_accuracy))
                print("current batch loss: %1.3f , fail count: %d" % (batch_loss, fail_count))                                                                                                
